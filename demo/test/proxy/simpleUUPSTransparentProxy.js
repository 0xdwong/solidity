...
describe("SimpleUUPSTransparentProxy", async () => {
    ...
    async function init() {
        ...
        // 部署逻辑合约
        {
            logicCA = await ethers.deployContract("SimpleUUPSLogic");
            logicCAAddr = await logicCA.getAddress();
        }

        // 部署代理合约
        {
            proxyCA = await ethers.deployContract("SimpleUUPSProxy", [logicCAAddr]);
            proxyCAAddr = await proxyCA.getAddress();

            proxyAsLogicCA = await ethers.getContractAt("SimpleUUPSLogic", proxyCAAddr);
        }
    }
